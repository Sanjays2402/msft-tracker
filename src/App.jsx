...
/* Microsoft 4-pane logo as SVG */
function MicrosoftLogo({ size = 52 }) {
  const gap = size * 0.06
  const pane = (size - gap) / 2
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      <rect x={0} y={0} width={pane} height={pane} rx={3} fill="#f25022" />
      <rect x={pane + gap} y={0} width={pane} height={pane} rx={3} fill="#7fba00" />
      <rect x={0} y={pane + gap} width={pane} height={pane} rx={3} fill="#00a4ef" />
      <rect x={pane + gap} y={pane + gap} width={pane} height={pane} rx={3} fill="#ffb900" />
    </svg>
  )
}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 8 }}>
          ...
          <div>
            <h1 style={{
              ...
              Microsoft <span style={{ color: 'var(--text-3)', fontWeight: 500, fontSize: 24 }}>MSFT</span>
            </h1>
